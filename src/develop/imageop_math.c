                          ...
                          const gboolean gamma)
{
  ...
  /* Both gammas must clamp at zero: powf() of a negative base with a
     fractional exponent is NaN, and a NaN here survives the whole pipe and
     shows up as a black pixel on screen. A -1e6f floor sanitized NaN only
     to feed powf() a negative base and get NaN straight back. */
  DT_OMP_SIMD(aligned(in, linear : 16))
  for(size_t k = 0; k < (size_t)roi_in->width * roi_in->height*4; k += 4)
    for_each_channel(c) linear[k+c] = powf(fmaxf(in[k+c], 0.0f), 2.4f);  // fmaxf sanitizes NaN
  ...
  /* The resampler is unclamped and every interpolator we offer bar bilinear
     ...
     -- lanczos3, the default, by as much as 12% of the step. */
  DT_OMP_SIMD(aligned(out : 16))
  for(size_t k = 0; k < (size_t)roi_out->width * roi_out->height * 4; k += 4)
    for_each_channel(c) out[k+c] = powf(fmaxf(out[k+c], 0.0f), 1.0f / 2.4f);
}
